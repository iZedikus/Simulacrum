...
public class SubmitDebitUseCase {
    ...
    public TransactionHistory execute(String consentId, String sourceAccountId, String recipientToken, BigDecimal amount, String currency) {
        ...
        var tx = new TransactionHistory(txId, sourceAccountId, TransactionStatusCode.Pending, BankTransactionCode.ObPayment, Instant.now(), Instant.now(), new Money(amount, currency), CreditDebitCode.Debit, recipientToken, "RECIPIENT", null, sourceAccountId, recipientToken);
        repo.save(tx);
        tx.complete();
        return repo.save(tx);
    }
